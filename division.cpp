#include <iostream>
#include <stdlib.h>
#include <omp.h>
#include <fstream>
using namespace std;

const unsigned int bit_size = 612;

// 'star', 612x612px
const unsigned char epd_bitmap_star[] = {
	
};

// 'hexagon', 612x612px
const unsigned char epd_bitmap_hexagon[] = {
	
};

const int epd_bitmap_allArray_LEN = 2;
const unsigned char* epd_bitmap_allArray[2] = {
	epd_bitmap_hexagon,
    epd_bitmap_star
};

unsigned char epd_bitmap_sum[bit_size*bit_size];

int main() {
    int i = 0;
    int x = 0;

    #pragma omp parallel shared(epd_bitmap_allArray) private(i) num_threads(x) reduction(+: epd_bitmap_sum)
    {
        #pragma omp for schedule(dynamic, 64)
        for (i = 0; i < bit_size*bit_size; i++) {
            if ((unsigned int)epd_bitmap_allArray[0][i] > 0) {
                epd_bitmap_sum[i] = (unsigned int)epd_bitmap_allArray[1][i] / (unsigned int)epd_bitmap_allArray[0][i];
            }
            else {
                epd_bitmap_sum[i] = (unsigned int)epd_bitmap_allArray[1][i];
            }
        }
    }

    ofstream myfile("Bitmaps/division.txt");
    if (myfile.is_open()) {
        for(int count = 0; count < bit_size*bit_size; count ++){
            myfile << std::hex << (unsigned int)(epd_bitmap_sum[count]) << ", " ;
        }
        myfile.close();
    }
    else cout << "Unable to open file";

    return 0;  
}